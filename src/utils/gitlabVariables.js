...
exports.GITLAB_PREDEFINED_VARIABLES = void 0;
exports.detectGitLabVariables = detectGitLabVariables;
exports.containsGitLabVariables = containsGitLabVariables;
exports.expandGitLabVariables = expandGitLabVariables;
exports.expandComponentUrl = expandComponentUrl;
exports.validateComponentUrlWithVariables = validateComponentUrlWithVariables;
exports.getVariableInfo = getVariableInfo;
 ...
 */
exports.GITLAB_PREDEFINED_VARIABLES = [
    {
        name: 'CI_API_V4_URL',
 ...
 */
function detectGitLabVariables(text) {
    ...
    const predefinedVariableNames = exports.GITLAB_PREDEFINED_VARIABLES.map(v => v.name);
 ...
 */
function containsGitLabVariables(text) {
 ...
 */
function validateComponentUrlWithVariables(url) {
    const variables = detectGitLabVariables(url);
    const unresolvedVariables = [];
    const suggestions = [];
    for (const variable of variables) {
        const varInfo = exports.GITLAB_PREDEFINED_VARIABLES.find(v => v.name === variable);
        if (varInfo) {
...
function getVariableInfo(variableName) {
    return exports.GITLAB_PREDEFINED_VARIABLES.find(v => v.name === variableName);
}
/**
 * Provides completion suggestions for GitLab variables
 */
function getVariableCompletions(prefix = '') {
    if (!prefix) {
        return exports.GITLAB_PREDEFINED_VARIABLES;
    }
    const upperPrefix = prefix.toUpperCase();
    return exports.GITLAB_PREDEFINED_VARIABLES.filter(v => v.name.includes(upperPrefix) || v.description.toLowerCase().includes(prefix.toLowerCase()));
}
//# sourceMappingURL=gitlabVariables.js.map