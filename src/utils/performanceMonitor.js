    ...
    async track(name, fn, metadata) {
        ...
        try {
            ...
            if (duration > SLOW_OPERATION_THRESHOLD_MS) {
                this.logger.warn(`[Performance] Slow operation detected: ${name} took ${duration}ms (threshold: ${SLOW_OPERATION_THRESHOLD_MS}ms)`, 'PerformanceMonitor');
                if (metadata) {
                    this.logger.debug(`[Performance] Operation metadata: ${JSON.stringify(metadata)}`, 'PerformanceMonitor');
                }
            }
            return result;
        }
