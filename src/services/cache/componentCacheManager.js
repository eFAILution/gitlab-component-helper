...
exports.ComponentCacheManager = void 0;
exports.getComponentCacheManager = getComponentCacheManager;
...
const versionCache_1 = require("./versionCache");
...
class ComponentCacheManager {
    constructor(context) {
        ...
        this.context = null;
        this.rawTemplateCache = new Map();
        this.logger.debug('[ComponentCache] Constructor called', 'ComponentCache');
        // Initialize specialized cache modules
        this.projectCache = new projectCache_1.ProjectCache();
        this.versionCache = new versionCache_1.VersionCache();
        this.groupCache = new groupCache_1.GroupCache(this.projectCache);
    ...
    async getComponents() {
        ...
        const cacheTime = config.get('cacheTime', 3600) * 1000; // Default 1 hour
    ...
    async fetchAndCacheRawTemplate(gitlabInstance, projectPath, filePath, version) {
        ...
        const config = vscode.workspace.getConfiguration('gitlabComponentHelper');
        const cacheTime = config.get('cacheTime', 3600) * 1000;
    ...
    addDynamicComponent(component) {
        try {
            ...
            const existingIndex = this.components.findIndex(comp => comp.name === component.name &&
    ...
    async refreshComponents() {
        return this.performanceMonitor.track('refreshComponents', async () => {
    ...
    async refreshComponentsInternal() {
        ...
        try {
            ...
            if (sources.length === 0) {
                ...
                const fetchPromises = sources.map(async (source) => {
                    try {
                        ...
                        const sourceType = source.type || cache_1.DEFAULT_COMPONENT_TYPE_PROJECT;
                        ...
                        if (sourceType === 'group') {
                            ...
                            return await this.groupCache.fetchComponentsFromGroup(gitlabInstance, source.path, source.name);
                        }
                        else {
                            // Original project-based fetching
                            return await this.projectCache.fetchComponentsFromProject(gitlabInstance, source.path, source.name);
                        }
                    }
                    catch (error) {
            ...
            const componentsNeedingVersions = this.components.filter(comp => !comp.availableVersions || comp.availableVersions.length === 0);
    ...
    addComponentToCache(component) {
        ...
        const existingIndex = this.components.findIndex(c => c.name === component.name &&
    ...
    async fetchComponentVersions(component) {
        try {
            ...
            const cachedComponent = this.components.find(c => c.name === component.name &&
                c.sourcePath === component.sourcePath &&
                c.gitlabInstance === component.gitlabInstance);
            if (cachedComponent) {
    ...
    async fetchSpecificVersion(componentName, sourcePath, gitlabInstance, version) {
        ...
        const existingComponent = this.components.find(c => c.name === componentName &&
        ...
        const cachedComponent = await this.projectCache.fetchSpecificVersion(componentName, sourcePath, gitlabInstance, version);
        if (cachedComponent) {
    ...
    shouldRefreshVersions() {
        ...
        const versionRefreshInterval = config.get('cacheTime', 3600) * 4 * 1000;
    ...
    getCacheInfo() {
        ...
        return {
            location: this.context ? cache_1.CACHE_LOCATION_GLOBAL_STATE : cache_1.CACHE_LOCATION_MEMORY_ONLY,
...
exports.ComponentCacheManager = ComponentCacheManager;
// Singleton instance
...
//# sourceMappingURL=componentCacheManager.js.map