...
class ProjectCache {
    ...
    async fetchComponentsFromProject(gitlabInstance, projectPath, sourceName) {
        ...
        try {
            const catalogData = await componentService.fetchCatalogData(gitlabInstance, projectPath, false);
            if (catalogData && catalogData.components) {
                this.logger.info(`[ProjectCache] Found ${catalogData.components.length} components in ${sourceName}`, 'ProjectCache');
                ...
                this.logger.debug(`[ProjectCache] Processed ${sourceComponents.length} components from ${sourceName}`, 'ProjectCache');
            ...
            this.logger.error(`[ProjectCache] Error fetching project ${projectPath}: ${errorMessage}`, 'ProjectCache');
            throw error;
        }
    }
    /**
    ...
    async fetchSpecificVersion(componentName, sourcePath, gitlabInstance, version) {
        try {
            this.logger.info(`[ProjectCache] Fetching specific version ${version} of ${componentName} from ${sourcePath}`, 'ProjectCache');
            const componentService = (0, component_1.getComponentService)();
            ...
            if (!availableVersions.includes(version)) {
                this.logger.warn(`[ProjectCache] Version ${version} does not exist. Available versions: ${availableVersions
                    .slice(0, 10)
                    .join(', ')}`, 'ProjectCache');
                return null;
            }
            ...
            if (!catalogData || !catalogData.components || catalogData.components.length === 0) {
                this.logger.warn(`[ProjectCache] No component data found for version ${version}`, 'ProjectCache');
                return null;
            }
            // Find the matching component in the catalog
            const catalogComponent = catalogData.components.find((c) => c.name === componentName);
            if (!catalogComponent) {
                this.logger.warn(`[ProjectCache] Component ${componentName} not found in version ${version}`, 'ProjectCache');
                return null;
            }
            ...
            const cachedComponent = {
                ...
                source: `${cache_1.SOURCE_COMPONENTS_PREFIX} ${sourcePath}`,
                sourcePath: sourcePath,
                gitlabInstance: gitlabInstance,
                version: version,
                url: `https://${gitlabInstance}/${sourcePath}/${catalogComponent.name}@${version}`,
            };
            this.logger.info(`[ProjectCache] Successfully fetched version ${version} of ${componentName}`, 'ProjectCache');
