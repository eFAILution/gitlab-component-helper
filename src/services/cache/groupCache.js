...
class GroupCache {
    ...
    async fetchComponentsFromGroup(gitlabInstance, groupPath, sourceName) {
        this.logger.info(`[GroupCache] Fetching projects from group: ${gitlabInstance}/${groupPath}`, 'GroupCache');
        try {
            // First, get all projects in the group
            const groupProjects = await this.fetchGroupProjects(gitlabInstance, groupPath);
            this.logger.info(`[GroupCache] Found ${groupProjects.length} projects in group ${groupPath}`, 'GroupCache');
            ...
            this.logger.debug(`[GroupCache] Checking ${groupProjects.length} projects for components (this may take a moment)...`, 'GroupCache');
            ...
            for (let i = 0; i < groupProjects.length; i += batchSize) {
                ...
                this.logger.debug(`[GroupCache] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(groupProjects.length / batchSize)} (projects ${i + 1}-${Math.min(i + batchSize, groupProjects.length)})`, 'GroupCache');
                const batchPromises = batch.map(async (project) => {
                    try {
                        this.logger.debug(`[GroupCache] Checking ${project.path_with_namespace}...`, 'GroupCache');
                        // Try to fetch components from this project using ProjectCache
                        const components = await this.projectCache.fetchComponentsFromProject(gitlabInstance, project.path_with_namespace, `${sourceName}/${project.name}`);
                        if (components.length > 0) {
                            this.logger.info(`[GroupCache] ✓ Found ${components.length} components in ${project.path_with_namespace}`, 'GroupCache');
                            return { project, components };
                        }
                        else {
                            this.logger.debug(`[GroupCache] - No components in ${project.path_with_namespace}`, 'GroupCache');
                            return { project, components: [] };
                        }
                    }
                    catch (error) {
                        // Log but don't fail the whole group if one project fails
                        this.logger.error(`[GroupCache] ✗ Error checking ${project.path_with_namespace}: ${error}`, 'GroupCache');
                        return { project, components: [] };
                    }
                });
            ...
            this.logger.info(`[GroupCache] Projects with components: ${projectsWithComponents}`, 'GroupCache');
            this.logger.info(`[GroupCache] Total components found: ${allComponents.length}`, 'GroupCache');
            return allComponents;
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            this.logger.error(`[GroupCache] Error fetching group ${groupPath}: ${errorMessage}`, 'GroupCache');
            throw error;
        }
    }
    /**
    ...
    async fetchGroupProjects(gitlabInstance, groupPath) {
        ...
        try {
            ...
            this.logger.debug(`[GroupCache] Using token for ${gitlabInstance}: ${token ? 'YES' : 'NO'}`, 'GroupCache');
            ...
            this.logger.info(`[GroupCache] Found ${projects.length} total projects in group ${groupPath}`, 'GroupCache');
            // Check all projects for components (no pre-filtering)
            this.logger.debug(`[GroupCache] Will check all ${projects.length} projects for components (no pre-filtering)`, 'GroupCache');
            return projects;
        }
        catch (error) {
...
//# sourceMappingURL=groupCache.js.map