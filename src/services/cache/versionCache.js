...
class VersionCache {
    constructor() {
        ...
        this.projectVersionsCache = new Map();
    }
    /**
    ...
    async fetchComponentVersions(component) {
        try {
            ...
            if (sortedVersions) {
                this.logger.info(`[VersionCache] [CACHE HIT] Reusing cached versions for project ${component.gitlabInstance}/${component.sourcePath}`, 'VersionCache');
            }
            else {
                ...
                this.logger.info(`[VersionCache] [API FETCH] Fetched ${sortedVersions.length} versions for project ${component.gitlabInstance}/${component.sourcePath}`, 'VersionCache');
            }
            this.logger.debug(`[VersionCache] Available versions for ${component.name}: ${sortedVersions
                .slice(0, 5)
                .join(', ')}${sortedVersions.length > 5 ? '...' : ''}`, 'VersionCache');
            return sortedVersions;
        }
        catch (error) {
            this.logger.error(`[VersionCache] Error fetching versions for ${component.name}: ${error}`, 'VersionCache');
    ...
    deserializeCache(data) {
        ...
        this.logger.debug(`[VersionCache] Restored ${this.projectVersionsCache.size} cached version entries`, 'VersionCache');
    }
    /**
