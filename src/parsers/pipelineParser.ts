...
export class PipelineParser {
    ...
    private async parseRecursive(content: string, sourceName: string, depth: number, parentNode: IncludeNode, context?: ParserContext, componentOrigin?: ComponentOrigin) {
        ...
        if (parsedObj.stages && Array.isArray(parsedObj.stages)) {
            for (const stage of parsedObj.stages) {
        ...
        for (const key of Object.keys(parsedObj)) {
            ...
            const jobObj = parsedObj[key];
            if (jobObj && typeof jobObj === 'object') {
                const stage = (jobObj as Record<string, any>).stage || 'test'; // default stage is test in GitLab CI
                this.allJobs.push({
                    name: key,
                    stage: stage,
                    source: sourceName
        ...
        // 3. Extract includes
        if (parsedObj.include) {
            const includes = Array.isArray(parsedObj.include) ? parsedObj.include : [parsedObj.include];
        ...
        if (parsedObj.pipeline_execution_policy && Array.isArray(parsedObj.pipeline_execution_policy)) {
            for (const policy of parsedObj.pipeline_execution_policy) {
                if (!policy) continue;
                ...
                const pipelineObj = pipelineDoc as Record<string, any>;
                // Extract stages declared inside the policy pipeline
                if (pipelineObj.stages && Array.isArray(pipelineObj.stages)) {
                    for (const stage of pipelineObj.stages) {
                        if (!this.customStages.includes(stage)) {
                            this.customStages.push(stage);
                        }
                    }
                ...
                for (const key of Object.keys(pipelineObj)) {
                    ...
                    const jobObj = pipelineObj[key];
                    if (jobObj && typeof jobObj === 'object') {
                        const stage = (jobObj as Record<string, any>).stage || 'test';
                        this.allJobs.push({ name: key, stage, source: policyLabel });
                    }
                }

                // Resolve includes declared inside the policy pipeline
                if (pipelineObj.include) {
                    const pepIncludes = Array.isArray(pipelineObj.include) ? pipelineObj.include : [pipelineObj.include];
