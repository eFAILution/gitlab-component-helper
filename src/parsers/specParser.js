...
exports.GitLabSpecParser = void 0;
...
class GitLabSpecParser {
    ...
    static parse(content, fileName) {
        ...
        try {
            ...
            if (typeof content !== 'string') {
                throw new errors_1.ParseError('Content must be a string', {
                    yaml: String(content).substring(0, 100)
                });
            }
            if (content.trim().length === 0) {
                throw new errors_1.ParseError('Content is empty', { yaml: content });
            }
            ...
            try {
                const specMatches = specSection.match(regex_1.SPEC_INPUTS_SECTION_REGEX);
                if (specMatches) {
                ...
                throw new errors_1.ParseError('Failed to parse spec inputs/variables', {
                    cause: error,
            ...
            // Wrap unknown errors
            throw new errors_1.ParseError(`Failed to parse GitLab component spec: ${error instanceof Error ? error.message : String(error)}`, {
                cause: error,
    ...
    static safeParse(content, fileName) {
        try {
            ...
            const parseError = error instanceof errors_1.ParseError
                ? error
                : new errors_1.ParseError(error instanceof Error ? error.message : String(error), { cause: error, yaml: content.substring(0, 500) });
...
exports.GitLabSpecParser = GitLabSpecParser;
