...
exports.getComponentUnderCursor = getComponentUnderCursor;
exports.detectIncludeComponent = detectIncludeComponent;
const vscode = __importStar(require("vscode"));
const component_1 = require("../services/component");
 ...
 */
async function detectComponent(document, position) {
    const line = document.lineAt(position.line).text;
    const componentService = (0, component_1.getComponentService)();
 ...
 */
async function getComponentUnderCursor(document, position) {
...
async function detectIncludeComponent(document, position) {
    const line = document.lineAt(position.line).text;
    ...
    if ((0, gitlabVariables_1.containsGitLabVariables)(componentUrl)) {
        const variables = (0, gitlabVariables_1.detectGitLabVariables)(componentUrl);
        ...
        if (gitContext.gitlabInstance && gitContext.projectPath) {
            ...
            expandedUrl = (0, gitlabVariables_1.expandComponentUrl)(componentUrl, context);
    ...
    try {
        ...
        if (requestedVersion) {
            const exactMatch = cachedComponents.find(comp => {
                ...
                return (comp.gitlabInstance === requestedGitlabInstance &&
 ...
 */
async function fetchComponentDynamically(componentUrl, originalUrl) {
    try {
        ...
        const foundComponent = catalogData.components.find((comp) => comp.name === componentName);
        if (!foundComponent) {
            ...
            logger.debug(`[ComponentDetector] Component ${componentName} not found in catalog. Available components: ${catalogData.components.map((c) => c.name).join(', ')}`, 'ComponentDetector');
        ...
        const dynamicComponent = {
            ...
            parameters: (foundComponent.variables || []).map((v) => ({
                name: v.name,
 ...
 */
async function getGitRepositoryContext() {
    try {
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders || workspaceFolders.length === 0) {
            return {};
        ...
        if (gitExtension) {
            ...
            if (git && git.repositories.length > 0) {
                const repo = git.repositories.find((r) => workspacePath.startsWith(r.rootUri.fsPath)) || git.repositories[0];
                if (repo) {
                    // Get remote URLs
 ...
 */
async function getRawGitRepositoryContext(workspacePath) {
    try {
        // Use VS Code's Git extension API if available
        const gitExtension = vscode.extensions.getExtension('vscode.git');
        if (gitExtension) {
 ...
 */
function parseAnyRemoteUrl(remoteUrl) {
    try {
        // Handle both HTTPS and SSH URLs
 ...
 */
async function detectNonGitLabRepository(workspaceFolder) {
    try {
        ...
        if (gitExtension) {
            ...
            if (git && git.repositories.length > 0) {
                ...
                let repo = git.repositories.find((r) => workspaceFolder.uri.fsPath.startsWith(r.rootUri.fsPath) ||
 ...
 */
async function detectRepositoryViaGitCommands(workspacePath) {
    try {
        // First, check if we're in a Git repository
        const isGitRepo = await new Promise((resolve) => {
            const gitCheck = (0, child_process_1.spawn)('git', ['rev-parse', '--is-inside-work-tree'], {
                cwd: workspacePath,
                stdio: 'pipe'
            });
            gitCheck.on('exit', (code) => {
                resolve(code === 0);
            });
            gitCheck.on('error', () => {
                resolve(false);
        ...
        const remoteUrl = await new Promise((resolve) => {
            const gitRemote = (0, child_process_1.spawn)('git', ['remote', 'get-url', 'origin'], {
                cwd: workspacePath,
                stdio: 'pipe'
            });
            let output = '';
 ...
 */
async function parseAndClassifyRepository(remoteUrl) {
    try {
 ...
 */
function parseNonGitLabRemoteUrl(remoteUrl) {
    try {
        // Handle both HTTPS and SSH URLs
        let hostname;
        let projectPath;
